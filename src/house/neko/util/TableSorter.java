...
public class TableSorter extends AbstractTableModel 
{
	...
	public int modelIndex(int viewIndex) 
	{
		...
		if(viewIndex < r.length && viewIndex > -1)
		{
	...
	private class SortableHeaderRenderer implements TableCellRenderer 
	{
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column)
		{
			...
			{	System.err.println("table.getColumnCount() <= column -- " + table.getColumnCount() + " <= "+ column); return null; }
			if(table.getRowCount() <= row)
			{	System.err.println("table.getRowCount() <= row -- " + table.getRowCount() + " <= "+ row); return null; }
			...
			if (c instanceof JLabel) 
			{
				...
				do
				{
					try
					{
						...
						System.err.println("guard="+guard + " -- " + npe.toString());
						npe.printStackTrace();
						try { Thread.sleep(10L); } catch(InterruptedException ie) { }
					} catch(ArrayIndexOutOfBoundsException aoobe) {
						System.err.println("guard="+guard + " -- " + aoobe.toString());
						aoobe.printStackTrace();
						try { Thread.sleep(10L); } catch(InterruptedException ie) { }
					}
				} while(modelColumn == null && --guard > 0);
