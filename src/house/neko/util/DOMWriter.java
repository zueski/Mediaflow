...
{
	private static String PRINTWRITER_ENCODING = "UTF8"; // Default Encoding 
	...
	private boolean qualifiedNames = true; // Output qualified names
	/**
	 *
	 */
	...
	public void print(Node node) 
	{
		...
		switch ( type ) 
		{
			...
			// print text
			case Node.TEXT_NODE: { out.print(normalize(node.getNodeValue())); break; }
	...
	{	
		...
		{
			String name = this.qualifiedNames ? array[i].getNodeName() : array[i].getLocalName();
			int index = i;
			for(int j = i + 1; j < len; j++)
			{
				String curName = this.qualifiedNames ? array[j].getNodeName() : array[j].getLocalName();
